...
dataframe = pd.read_excel('Farming\AgriculturalT.xlsx', engine='openpyxl')
NaN_2021=np.where(dataframe['2021']==0)
NaN_2020=np.where(dataframe['2020']==0)
NaN_2019=np.where(dataframe['2019']==0)
NaN_2018=np.where(dataframe['2018']==0)
delete_col = ['subjects_of_the_RF', 'label']
...
while i < len(NaN_2021[0]):
    for pred_object in NaN_2021:
        df = dataframe.drop(delete_col, axis = 1,  inplace=False)
        methods = {'pred_kk_by_feature': pred_kk_by_feature(df, pred_object[i], '2021'),
                   'pred_kk_by_object': pred_kk_by_object(df, pred_object[i], '2021'), 
                   'pred_metric_euclid': pred_metric('euclid', df, pred_object[i], '2021'), 
                   'pred_metric_manhatten': pred_metric('manhatten', df, pred_object[i], '2021'), 
                   'pred_metric_max_metric': pred_metric('max_metric', df, pred_object[i], '2021')}
        pred = []
        for value in methods.values():
            pred.append(round(value, 2))
        pred.sort()
...
while i < len(NaN_2020[0]):
    for pred_object in NaN_2020:
        df = dataframe.drop(delete_col, axis = 1,  inplace=False)
        methods = {'pred_kk_by_feature': pred_kk_by_feature(df, pred_object[i], '2020'),
                   'pred_kk_by_object': pred_kk_by_object(df, pred_object[i], '2020'), 
                   'pred_metric_euclid': pred_metric('euclid', df, pred_object[i], '2020'), 
                   'pred_metric_manhatten': pred_metric('manhatten', df, pred_object[i], '2020'), 
                   'pred_metric_max_metric': pred_metric('max_metric', df, pred_object[i], '2020')}
        pred = []
        for value in methods.values():
            pred.append(round(value, 2))
        pred.sort()
        dataframe.loc[pred_object[i], '2020'] = round(statistics.median(pred), 2)
    i += 1

i = 0
while i < len(NaN_2019[0]):
    for pred_object in NaN_2019:
        df = dataframe.drop(delete_col, axis = 1,  inplace=False)
        methods = {'pred_kk_by_feature': pred_kk_by_feature(df, pred_object[i], '2019'),
                   'pred_kk_by_object': pred_kk_by_object(df, pred_object[i], '2019'), 
                   'pred_metric_euclid': pred_metric('euclid', df, pred_object[i], '2019'), 
                   'pred_metric_manhatten': pred_metric('manhatten', df, pred_object[i], '2019'), 
                   'pred_metric_max_metric': pred_metric('max_metric', df, pred_object[i], '2019')}
        pred = []
        for value in methods.values():
            pred.append(round(value, 2))
        pred.sort()
        dataframe.loc[pred_object[i], '2019'] = round(statistics.median(pred), 2)
    i += 1

i = 0
while i < len(NaN_2018[0]):
    for pred_object in NaN_2018:
        df = dataframe.drop(delete_col, axis = 1,  inplace=False)
        methods = {'pred_kk_by_feature': pred_kk_by_feature(df, pred_object[i], '2018'),
                   'pred_kk_by_object': pred_kk_by_object(df, pred_object[i], '2018'), 
                   'pred_metric_euclid': pred_metric('euclid', df, pred_object[i], '2018'), 
                   'pred_metric_manhatten': pred_metric('manhatten', df, pred_object[i], '2018'), 
                   'pred_metric_max_metric': pred_metric('max_metric', df, pred_object[i], '2018')}
        pred = []
        for value in methods.values():
            pred.append(round(value, 2))
        pred.sort()
        dataframe.loc[pred_object[i], '2018'] = round(statistics.median(pred), 2)
    i += 1

dataframe.to_excel('Farming\AgriculturalTR.xlsx', index=True);